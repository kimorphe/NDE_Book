...
\begin{eqnarray}
	...
	\delta\left(\fat{r}_i(\fat{x})+r_j(\fat{x})-r_i(k)-r_j(k)\right) 
...
\begin{equation}
	...
	\label{eqn:Gauss}
\end{equation}
で近似する．$\delta_s(s)$は普通の関数であり数値的な評価は容易である．また，$\sigma\rightarrow 0$の
極限においてデルタ関数を与えるため，パラメータ$\sigma$の設定により近似の精度を必要なだけ上げることができる．
...
\begin{equation}
	...
	\delta_\sigma\left(\fat{r}_i(\fat{x})+\fat{r}_j(\fat{x})-r_i(k)-r_j(k)\right) 
...
\begin{equation}
	...
	\label{eqn:}
\end{equation}
と散乱体強度に一致することが確かめられる．一方，$\fat{x}\neq \fat{z}_k$の場合，
$I_{\sigma}(\fat{x})$がどのような値を取るかは明らかでない．しかしながら，$I_\sigma(\fat{x})$が1に近い値を
取るのはレアなケースであると予想されることが次のようにして分かる．
はじめに，$\sigma$が十分小さい時，$\delta_\sigma(s)$は$|s|\rightarrow \infty$に
おいて急速に減少する関数であることから，
$\delta_\sigma\left(\fat{r}_i(\fat{x})+\fat{r}_j(\fat{x})-r_i(k)-r_j(k)\right)$が
有意な値を取るは
\begin{equation}
	\fat{r}_i(\fat{x})+\fat{r}_j(\fat{x})-r_i(k)-r_j(k)=0
	\label{eqn:cntr}
\end{equation}
のごく近傍に限られる．式(\ref{eqn:cntr})は$(i,j)$組，すなわち送受信点のペアに対して
3次元問題では一つの曲面を， 2次元問題では曲線を定義する．
従って，$(i,j)$を一つ指定したとき，式(\ref{eqn:cntr})を満足するような$\fat{x}$は
無数に存在する．しかしながら，$I_\sigma(\fat{x})$が1に近い値をとるためには，
多数の$(i,j)$の組に対して式(\ref{eqn:cntr})を満足する必要がある．つまり，
$\fat{x}$は多数の曲面あるいは曲線の共通部分に入っている必要がある．
2つの曲面の共通部分は一般に曲線で，2つの曲線の共通部分は点であることを
考えると，そのようなケースは稀であると予想されるため．
が共通部分を持つ場合，共通部分は曲線となることが一般的で
これは$\left| {\cal A} \right|$が大きいとき，非常に


標準偏差適当な連続関数$\delta_\sigma(s)$で近似する．
ただし，$\left\{ \delta_\sigma(s) \right\} $はパラメータ$\sigma(>0)$を持つ
関数の族で$\sigma\rightarrow 0 $の極限でデルタ関数を与えるものとする．
例えば，

このデルタ関数はグリーン関数に由来するもので，入射波形をインパルス波形とした
ことによる．実際の計測では，送信センサーは帯域に制限があるため，デルタ関数が
帯域制限
