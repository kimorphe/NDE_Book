...
\end{eqnarray}
ここに，$U^{in}$と$U^{sc}$は，$u^{in}$と$u^{sc}$の時間に
関するフーリエ変換をそれぞれ表す．
以下では，開口合成法における画像化関数$I(\fat{x})$が，${\cal I}(\fat{x})$の
特別なケースとして得られることを示す．
%
なお，散乱場$u^{sc}$は，1次元問題のようにを観測波形だけを使って具体的に
表現することができない．そこで，観測面における条件が指定され，遠方近似が
成り立つと仮定してこの後の議論を進める．
%
欠陥が存在しない場合の領域を$B$その表面を$\partial B$とし，
観測点$\cal M$は$\partial B$上に置かれているとする．
欠陥が占める領域とその表面はこれまで通り$D$および$\partial D$ですれぞれ表す．
全ての境界$\partial D\cup \partial B$において，次のノイマン条件を課す．
\begin{equation}
...
\begin{equation}
	...
	\label{eqn:}
\end{equation}
を満足するものとする．また，全空間のグリーン関数$G^+$と$G^-$を
次のような特異解として定義する．
\begin{equation}
	\nabla^2_x G^{\pm}(\fat{x},\fat{y},t)
	-
	\frac{1}{c^2}\ddot{G}^{\pm}(\fat{x},\fat{y},t)
	=
	-4\pi \delta(\fat{x}-\fat{y})\delta(t)
	\label{eqn:XXX_Green_pm}
\end{equation}
ここで，$G^+$と$G^-$はそれぞれ因果的，非因果的なグリーン関数を表す．
\begin{equation}
	G^{\pm}(\fat{x},\fat{y},t)=\frac{1}{r}\delta \left(t\pm \frac{|\fat{x}-\fat{y}|}{c}\right)
	\label{eqn:}
\end{equation}
\begin{equation}
	u(\fat{x},t) 
...
散乱場$u^{sc}(\fat{x},t)$を，$u^{sc}=u-u^{in}$で定義する．
入射場$u^{in}(\fat{x},t)$を，欠陥$D$が存在しないときの波動場を，
散乱場$u^{sc}(\fat{x},t)$を，$u^{sc}=u-u^{in}$で定義する．
散乱場$u^{sc}(\fat{x},t)$は，全波動場と同様，境界上にノイマン条件$u$
全空間のグリーン関数$G(\fat{x},\fat{y},t)$を用いて次のよう表すことができる．
\begin{equation}
	u^{sc}(\fat{y},t)
	=
	-\int_{\Gamma}
	\frac{\partial G(\fat{x},\fat{y},t)}{\partial n_x}*u(\fat{x},t)
	dS_x	
	\label{eqn:}
\end{equation}
ただし$*$は，時間に関するたたみ込み積を意味する．
