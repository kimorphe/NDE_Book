...
この円の上に等間隔で$M$個の送受信点を配置する。これら$M$個の送受信点の組の中には、
等時曲線の法線ベクトルが一致するものが多数含まれる。そこで、法線ベクトルの重複を避ける
ため、パルスエコーモードでの観測データ${\cal D}_M$を用いる。
このとき、$\fat{n}_{ii}$は、単位円上に等間隔で配置され、互いに一致することはない。
