...
\begin{equation}
	...
	\label{eqn:}
\end{equation}
を用いて$U^{in}\left( U^{sc}\right)^*$の$\omega$に関する積分を計算すると，
\begin{equation}
	\left< U^{in},U^{r}\right>(x)
	=
	\int 
	U^{in}\left( U^{r}\right)^* d\omega
	=
	\int 
...
\begin{equation}
	...
	\label{eqn:}
\end{equation}
より，式(\ref{eqn:pcnj})は$x=L$で最大値を取ることが分かる．
そこで、$<U^{in},U^{r}>$を位置の関数として見ることで、
反射壁表面を画像化できる。これは、以下の理由から開口合成法の一般化とみなすことができる．
