...
与えれば実際に計算することができ，$I_\sigma(\fat{x})$の値は，曲線群(\ref{eqn:XXX_Crvs})
の配置によって概ね決定される．具体的には，多数の曲線が交差する箇所で大きな値をとり，
少数の曲線が通る，あるいは曲線が通らない箇所は小さな値をとる．
...
$a_{i'j'}(t)$があったとしても，もし$\fat{\xi}_{ij}\simeq \fat{\xi}_{i'j'}$
ならば，どちらか一方を使えば十分で，二つを用いることによって
散乱体位置特定の解像度をあげる効果は期待できない．
このような状況は頻繁に生じる．
このことを簡単な例で見てみる．
...
図\ref{fig:XXX_blind_sum}(a)のような状況で$M=2$の観測データ${\cal D}({\cal M}^2)$を得たとする．
